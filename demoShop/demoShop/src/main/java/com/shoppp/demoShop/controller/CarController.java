...
public class CarController
{
    ...
    @GetMapping("/carBy/{category}")
    public List<Car> getCarsByCategory(@PathVariable("category") Category category)
    {
        return  carService.getCarsByCategory(category);
    }

    @PutMapping("/car/{id}")
    @ApiOperation(value = "updates car", httpMethod = "PUT http://localhost:8080/car/{id}", consumes = "car json and cars id")
